...
function ScrollableBox(options) {
  ...
  function createScrollSpeed() {
    let prev = new Date();
    return () => {
      const now = new Date();
      const lines = now - prev < 20 ? 3 : 1;
      prev = now;
      return lines;
    };
  }

  const scrollSpeed = createScrollSpeed();
...
ScrollableBox.prototype.scroll = function(offset, always) {
  ...
  // Parsing big content on each scroll really slows down performance.
  //
  //
