...
var custPth = ISPROD ? require("./vjuh-usr").path : path.resolve(__dirname, '..', 'usr/');
...
Tput.prototype._useXtermCap = function() {
  return this.injectTermcap(path.join(custPth, 'xterm.termcap'));
};

Tput.prototype._useXtermInfo = function() {
  return this.injectTerminfo(path.join(custPth, 'xterm'));
};

Tput.prototype._useInternalInfo = function(name) {
  return this.injectTerminfo(path.join(custPth, path.basename(name)));
};

Tput.prototype._useInternalCap = function(name) {
  return this.injectTermcap(path.join(custPth, path.basename(name) + '.termcap'));
};
